...
//! under `AppState::write_lock`, each library under its own registry slot, grants, OAuth and title metadata under
//! their own — so one record has one writer at a time, but different owners write at once. What they share is the cap,
