...
pub struct AppState {
    ...
    /// Allowlisted TMDB poster metadata observed by paired clients. This is separate from Atlas and from the
    /// full-response proxy cache: Atlas never transports ratings, and this shared index exists even when the
    /// household has no TMDB key configured.
    pub tmdb_metadata_cache_dir: Option<std::path::PathBuf>,
    /// Serializes the small read/merge/write records so simultaneous partial observations cannot erase fields.
    pub tmdb_metadata_writes: tokio::sync::Mutex<()>,
...
impl AppState {
    pub fn new(store: store::Store, metrics_token: Option<String>, log_requests: bool) -> Self {
        AppState {
            ...
            tmdb_metadata_cache_dir: None,
            tmdb_metadata_writes: tokio::sync::Mutex::new(()),
...
async fn main() {
    ...
    state.tmdb_metadata_cache_dir = Some(std::path::Path::new(&dir).join("tmdb-metadata"));
    ...
    tokio::spawn(tmdb_metadata::sweep_forever(Arc::clone(&state)));
