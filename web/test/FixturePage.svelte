...
<section style="padding-top:80px" data-page={route.page} data-route-id={route.page === 'title' || route.page === 'person' ? route.id : undefined}>
  <h1>{route.page}</h1>
  ...
  <div class="rail" style="display:flex;width:100%;overflow-x:auto;height:120px;">
    {#each Array(count) as _, i}<div style="flex:0 0 200px">{i}</div>{/each}
  </div>
