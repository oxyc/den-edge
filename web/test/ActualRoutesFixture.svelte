<script lang="ts">
 ...
 import '../src/app.css';
 const noop=()=>{};
 const log={settings:(group:string)=>group==='keys'?{values:{tmdb:{value:{string:'fixture-key'},at:[1,0,'test']}}}:undefined,refresh:async()=>false,rows:()=>[],newestStamp:()=>[1,0,'test'],title:()=>undefined};
 const session={changed:()=>{},revision:0,settingsRevision:0,displays:[],shapes:new Map(),log,opened:Promise.resolve(log)} as unknown as LibrarySession;
