...
test('desktop Back traverses nested details and direct links have a Home fallback',async()=>{
  ...
  try {
    ...
    await expect.poll(()=>page.getByRole('link',{name:'Den home'}).locator('img').evaluate(img=>img.naturalWidth)).toBeGreaterThan(0);
    ...
    for(const width of [320,390]) {
      ...
      await page.evaluate(()=>document.dispatchEvent(new CustomEvent('den:navigate',{detail:'#title/movie/9'})));
