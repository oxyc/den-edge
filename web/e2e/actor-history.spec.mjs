  ...
  try {
    ...
    await page.route('https://api.themoviedb.org/**',async r=>{
      ...
      if(url.includes('combined_credits')) {
        await filmsReady;
        return r.fulfill({json:{cast:Array.from({length:24},(_,i)=>({id:100+i,media_type:'movie',title:'Actor film '+i,poster_path:'/poster.jpg',release_date:'2020-01-01',vote_count:1000,popularity:100-i}))}});
      }
      if(url.includes('/person/')) return r.fulfill({json:{id:7,name:'An Actor',profile_path:'/actor.jpg',biography:'Actor biography. '.repeat(100)}});
      return r.fulfill({json:{id:42,title:'The Movie',poster_path:'/poster.jpg',backdrop_path:'/backdrop.jpg',overview:'A long description. '.repeat(100),credits:{cast:[{id:7,name:'An Actor'}]},recommendations:{results:[]}}});
    });
    ...
    await page.evaluate(()=>{
      ...
      for(const [type,x] of [['touchstart',innerWidth-5],['touchmove',innerWidth-150],['touchend',innerWidth-250]]) {
    ...
    expect(frozen.equals(live),'actor Forward snapshot must match live content after background loading and rotation').toBe(true);
