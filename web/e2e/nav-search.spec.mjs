...
const film = (id, title = `Film ${id}`) => ({ id, title, media_type: 'movie', release_date: '2026-01-01', poster_path: '/poster.jpg', backdrop_path: '/backdrop.jpg', genre_ids: [18], vote_average: 8, vote_count: 1000, popularity: 100 });
...
async function setup(page, { atlasGate, catalogueGate, searchGate } = {}) {
  ...
  await page.route('**/atlas/manifest.json', async r => {
    ...
    await atlasGate; return r.fulfill({ json: { id: 'com.den.atlas', catalogs: [] } });
  });
  await page.route('**/atlas/catalog/**', async r => {
    await catalogueGate; return r.fulfill({ json: { metas: [] } });
  });
  await page.route('https://image.tmdb.org/**', r => r.fulfill({ contentType: 'image/svg+xml', body: '<svg xmlns="http://www.w3.org/2000/svg" width="500" height="281"><rect width="500" height="281" fill="#264c68"/></svg>' }));
  await page.route('https://api.themoviedb.org/**', async r => {
    const url = new URL(r.request().url());
    if (url.pathname.includes('/search/')) {
      const q = url.searchParams.get('query'); queries.push(q);
      await searchGate?.(q);
      return r.fulfill({ json: { results: q.startsWith('empty') ? [] : q.startsWith('Slow') ? [film(999, 'Slow result')] : films, total_pages: 1 } });
    }
    const match = /\/(movie|tv)\/(\d+)$/.exec(url.pathname);
    if (match) return r.fulfill({ json: { ...film(Number(match[2])), overview: 'A movie description.', genres: [{ id: 18, name: 'Drama' }], credits: { cast: [] }, recommendations: { results: [] } } });
...
for (const width of [320, 390, 1280]) test(`navbar search preserves Home and result history at ${width}px`, async () => {
  const browser = await chromium.launch({ executablePath: process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH });
  try {
    const page = await browser.newPage({ viewport: { width, height: 800 }, hasTouch: width < 760, reducedMotion: 'reduce' });
    let documents = 0; const errors = [];
    ...
    await page.evaluate(() => { window.homeHero = document.querySelector('.billboard'); scrollTo(0, 700); });
    const homeY = await page.evaluate(() => scrollY); expect(homeY).toBeGreaterThan(300);
    ...
    expect(await page.evaluate(() => document.querySelector('[data-active="true"] .billboard') === window.homeHero)).toBe(true);
    await expect(active(page).locator('.billboard .dot').nth(1)).toHaveAttribute('aria-current', 'true');
    await expect(page.getByRole('link', { name: 'Settings', exact: true })).toBeVisible();
    if (width < 760) {
    ...
    await expect(active(page).getByText('Search movies, series and people.', { exact: true })).toBeVisible();
    expect(errors).toEqual([]); expect(documents).toBe(1);
...
test('a late search cannot replace a newer query', async () => {
  ...
  try {
    ...
    const queries = await setup(page, { searchGate: q => q.startsWith('Slow') ? slow : Promise.resolve() });
...
test('late discovery keeps the already visible billboard and selected slide', async () => {
  ...
  try {
    ...
    const atlasGate = new Promise(r => releaseAtlas = r), catalogueGate = new Promise(r => releaseCatalogue = r);
    ...
    await page.evaluate(() => {
      ...
      new MutationObserver(() => { if (!document.querySelector('.billboard .slide') || !document.querySelector('.billboard .backdrop.lit')) window.heroBlanked = true; }).observe(document.querySelector('.billboard'), { subtree: true, childList: true, attributes: true });
    });
    const fetching = page.waitForRequest('**/atlas/catalog/movie/jw-trending.json');
    releaseAtlas(); await fetching;
    ...
    releaseCatalogue(); await response;
...
for (const width of [320, 393, 1280]) test(`billboard opens by mobile slide tap and keeps desktop actions at ${width}px`, async () => {
  const browser = await chromium.launch({ executablePath: process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH });
  try {
    const page = await browser.newPage({ viewport: { width, height: 800 }, hasTouch: width < 760, reducedMotion: 'reduce' });
    await setup(page);
    ...
    await expect(hero.locator('.slide').first()).toBeVisible();
    if (width >= 760) {
      await expect(hero.locator('.slide').first().getByRole('link', { name: 'More', exact: true })).toBeVisible();
    ...
    await cdp.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [{ x: start, y: 220 }] });
    for (let i = 1; i <= 6; i++) {
      await cdp.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [{ x: start - i * (start - 50) / 6, y: 220 }] });
