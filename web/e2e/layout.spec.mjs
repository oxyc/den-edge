...
test('layout regressions', async () => {
  ...
  try {
    ...
    for (const width of [390, 1280]) {
      ...
      await detailPage.route('https://image.tmdb.org/**', async (route) => {
        await pictures;
        await route.fulfill({
          contentType: 'image/svg+xml',
          body: '<svg xmlns="http://www.w3.org/2000/svg" width="200" height="300"><rect width="200" height="300" fill="blue"/></svg>',
      ...
      assert.ok(
        Math.abs(loaded.height - before.height) < 2,
        'hero should reserve its loading height',
      );
      const poster = await detailPage.locator('.poster').boundingBox();
      assert.ok(
        Math.abs(poster.height - poster.width * 1.5) < 1,
        'poster space must be reserved before image load',
      );
      releaseImages();
      await detailPage.waitForFunction(() => Array.from(document.images).every((i) => i.complete));
