    ...
    try {
      for (const width of [390, 1280]) {
        ...
        await page.waitForFunction(
          () =>
            document.querySelector('[data-active="true"] [data-page="library"]') && scrollY > 800,
        );
        ...
        await page.evaluate(() => {
          ...
          for (const [type, x] of [
            ...
            if (type === 'touchstart' && allowed)
              throw Error('App must claim the edge before native history starts');
            if (type === 'touchmove') {
              const overlay = document.querySelector('[data-swipe-preview]');
              if (
                !overlay ||
                overlay.children[0].querySelector('h1').textContent !== 'library' ||
                overlay.children[1].querySelector('h1').textContent !== 'title'
              )
                throw Error('Swipe must reveal the previous page while dragging');
              if (overlay.children[0].querySelector('input').value !== 'my query')
                throw Error('Previous snapshot lost search state');
            }
          }
        });
        await page.waitForFunction(
        ...
        for (const cancel of [true, false]) {
          await page.evaluate((cancel) => {
            ...
            for (const [type, x] of [
              ...
              ['touchmove', innerWidth - 45],
              ['touchend', innerWidth - (cancel ? 50 : 150)],
            ]) {
              const allowed = target.dispatchEvent(
                new TouchEvent(type, {
                  bubbles: true,
              ...
              if (type === 'touchmove') {
                const overlay = document.querySelector('[data-swipe-preview]');
                if (!overlay || overlay.children[0].querySelector('h1').textContent !== 'title')
                  throw Error('Forward target must be visible during the leftward drag');
                if (overlay.children[1].getBoundingClientRect().x >= 0)
                  throw Error('Current snapshot must move left');
              }
            }
          }, cancel);
          await page.waitForSelector('[data-swipe-preview]', { state: 'detached' });
        ...
        await page.waitForFunction(
          () =>
            document.querySelector('[data-active="true"] [data-page="library"]') && scrollY === 950,
        );
        await page.waitForTimeout(260);
        ...
        assert.deepEqual(
          await page.evaluate(() => window.transitionErrors),
          [],
          'ordinary transitions must not time out',
        );
        ...
        console.log(
          `${name} ${width}: nested back/forward, query, loaded rows, horizontal scroll, tab return, swipe-back and swipe-forward/cancel passed`,
        );
        await context.close();
      }
      for (const mode of ['reduced-motion', 'unsupported', 'rapid']) {
        ...
        await page.goBack();
        await page.waitForFunction(() =>
          document.querySelector('[data-active="true"] [data-page="title"]'),
        );
        await page.waitForTimeout(250);
        await page.goBack();
        await page.waitForFunction(
      ...
      }
      {
        ...
        const overlays = await page.evaluate(() => {
          const target = document.querySelector('[data-active="true"] section');
          const send = (type, x, id) => {
            const touch = new Touch({ identifier: id, target, clientX: x, clientY: 300 });
          ...
          const button = Array.from(target.querySelectorAll('button')).find(
            (b) => b.textContent === 'Person',
          );
          const ghostAllowed = button.dispatchEvent(
          ...
          return {
            count: document.querySelectorAll('[data-swipe-preview]').length,
            endAllowed,
            ghostAllowed,
          };
        });
        ...
        assert.equal(
          overlays.ghostAllowed,
          false,
          'swipes must not also click through to a title or person',
        );
      ...
      }
      {
        ...
        for (const [from, to, selector, expected] of [
          ...
          await page.waitForFunction(
            (expected) =>
              document.querySelector('[data-active="true"] h1')?.textContent === expected,
            expected,
          );
          await page.waitForSelector('[data-swipe-preview]', { state: 'detached' });
        }
        const unaffected = await page.evaluate(() => {
          ...
          return {
            vertical: send('section', [
              ['touchstart', 100, 300],
              ['touchmove', 102, 360],
              ['touchend', 102, 400],
            ]),
            carousel: send('.rail div', [
              ['touchstart', 100, 300],
              ['touchmove', 220, 300],
              ['touchend', 240, 300],
            ]),
        ...
        assert.deepEqual(unaffected, {
          ...
          carousel: [true, true, true],
          overlays: 0,
        });
        console.log(
          'interior back/forward and carousel-edge history passed; vertical scrolling and interior carousel gestures remain native',
        );
        await context.close();
      }
      {
        ...
        assert.equal(
          heading,
          'library',
          'movie 1 must never be saved as the Home snapshot while a loading cover is visible',
        );
        await context.close();
      ...
      }
      {
        ...
        assert.equal(
          ...
          0,
          'opening a movie from scrolled Home starts at top',
        );
        ...
        assert.equal(
          await page.evaluate(() => scrollY),
          0,
          'explicitly reopening a movie starts at top, even if cached',
        );
        await context.close();
        console.log(
          'fresh/reopened detail starts at top; Home and history traversal retain their scroll',
        );
      }
      {
        const context = await browser.newContext({
      ...
      }
      {
        ...
        for (const id of [41, 42, 43]) {
          ...
          await page.waitForFunction(
            (id) =>
              document.querySelector('[data-active="true"] section')?.dataset.routeId ===
              String(id),
            id,
          );
          await page.goBack();
          await page.waitForFunction(
            () => document.querySelector('[data-active="true"] h1')?.textContent === 'library',
          );
        }
        await context.close();
