...
for (const width of [390, 1280])
  test(`detail trailer layout and lifecycle at ${width}px`, async () => {
    ...
    try {
      ...
      const geometry = () =>
        page.evaluate(() =>
          ['.hero', '.head', '.hero-actions', '.overview'].map((s) => {
      ...
      await expect(video).not.toHaveAttribute('src');
      await page.evaluate(() =>
        document.dispatchEvent(new CustomEvent('fixture:active', { detail: true })),
      );
      await expect(video).toHaveClass(/\bplaying\b/);
      expect(await video.evaluate((v) => v.currentTime)).toBeLessThan(3);
...
for (const reduced of [false, true])
  test(`detail trailer still fallback: ${reduced ? 'reduced motion' : 'unavailable'}`, async () => {
    const browser = await chromium.launch({
      executablePath: process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH,
    });
    try {
...
for (const failure of ['media error', 'autoplay blocked'])
  test(`detail trailer still fallback: ${failure}`, async () => {
    const browser = await chromium.launch({
      executablePath: process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH,
    });
    try {
      ...
      const video = page.locator('video');
      if (failure === 'media error')
        await expect.poll(() => video.evaluate((v) => !!v.error)).toBe(true);
...
test('wide mobile trailer and its swipe snapshot have opaque letterboxing', async () => {
  ...
  try {
    ...
    const clip = {
      ...
      width: 1,
      height: Math.floor(bounds.height) - 1,
    };
    const samples = async () => {
      ...
      return page.evaluate(async (data) => {
        ...
        return [5, Math.floor(img.height / 2), img.height - 6].map((y) =>
          ...
          await page.route('https://api.themoviedb.org/**', (r) =>
            r.fulfill({
              json: {
                ...
                videos: {
                  results: [
                    { site: 'YouTube', type: 'Trailer', official: true, key: 'fixture-key' },
                  ],
                },
              },
            }),
...
for (const failed of ['missing', 'portrait'])
  test(`mobile trailer tries the next candidate after a ${failed} first trailer`, async () => {
    const browser = await chromium.launch({
      executablePath: process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH,
    });
    try {
