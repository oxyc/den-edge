...
describe('routes', () => {
  it('reads each page from the fragment', () => {
    ...
    expect(parseRoute(titleHref({ type: 'movie', id: 550 }))).toEqual({ page: 'title', type: 'movie', id: 550 });
    expect(parseRoute(personHref(287))).toEqual({ page: 'person', id: 287 });
  });

  it('reads anything else as the library', () => {
    for (const other of ['', '#', '#library', '#title/person/1', '#title/movie/-3', '#title/movie/1.5', '#person/x', '#pair=ABCD']) {
