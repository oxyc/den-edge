...
export const routeKey = (route: Route): string =>
  route.page === 'title' ? `title/${route.type}/${route.id}` : route.page === 'person' ? `person/${route.id}` : route.page;

/** Retained page state: details own a history visit; top-level tabs reuse their browsing surface. */
export interface PageVisit { key: string; route: Route; x: number; y: number }
export class Navigation {
  readonly pages = new Map<string, PageVisit>();
  current: PageVisit;
  constructor(hash: string) { this.current = this.visit(hash); }
  ...
  prune(retained: ReadonlySet<string>) {
    for (const [key, page] of this.pages) {
      if (page !== this.current && !retained.has(key) && (page.route.page === 'title' || page.route.page === 'person')) {
        this.pages.delete(key);
      }
    }
  }
  save(x: number, y: number) { this.current.x = x; this.current.y = y; }
}
