...
vi.mock('./scout', () => ({ findAddon: vi.fn(), findAtlas: vi.fn(), SCOUT: { name: 'scout' }, REEL: { name: 'reel' } }));
vi.mock('./remux', () => ({ findRemux: vi.fn() }));

beforeEach(() => {
  vi.mocked(findAddon).mockImplementation(async (_installed, _routes, kind) => ({ base: `/${kind.name}`, install: `/${kind.name}` }));
...
it('publishes discovery addons while the playback health request is still pending', async () => {
  ...
  vi.mocked(findRemux).mockReturnValue(new Promise((done) => { resolve = done; }));
