...
it('requires a deliberate rightward swipe, not left, short, vertical or diagonal motion', () => {
  ...
  for (const [x, y] of [[-100, 0], [40, 0], [100, 100], [5, 180], [100, -80]]) {
...
function gestureHarness(rail = false) {
  ...
  const element = { closest: () => null, parentElement: null, scrollWidth: rail ? 800 : 390, clientWidth: 390 };
  const document = new EventTarget();
  const preview = { move: vi.fn(), finish: vi.fn(async () => {}), release: vi.fn(async () => {}), dispose: vi.fn() };
  const destination = () => ({ canNavigate: () => true, navigate: vi.fn(), preview: vi.fn(() => preview) });
  const back = destination(), forward = destination();
...
it('does not traverse a changed history entry when navigation interrupts a finishing swipe', async () => {
  ...
  let finish!: () => void;
  preview.finish.mockImplementation(() => new Promise<void>(resolve => { finish = resolve; }));
  send('touchstart', 5);
  send('touchmove', 100);
  send('touchend', 150);
