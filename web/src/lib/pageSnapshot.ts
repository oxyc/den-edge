...
export interface PageSnapshot { show(): HTMLElement; refresh(): PageSnapshot }
export function capturePage(source = document.querySelector<HTMLElement>('[data-route-page][data-active="true"]'), rootStyle?: string): PageSnapshot | null {
  ...
  const offsets = new Map(copies.map((node, i) => [node, { x: originals[i]!.scrollLeft, y: originals[i]!.scrollTop }]));
  copies.forEach((node, i) => {
    ...
    if (original) {
      ...
      for (const property of ['transform', 'translate', 'rotate', 'scale', 'opacity', 'filter', 'backdrop-filter']) {
  ...
  copies.forEach((node, i) => {
    ...
    if (original.readyState >= 2 && original.videoWidth && original.videoHeight && Number(painted.opacity) > 0 &&
      ...
      for (const property of painted) canvas.style.setProperty(property, painted.getPropertyValue(property));
      ...
      } catch { /* A decoder without a readable frame keeps the still artwork underneath. */ }
    }
    node.remove();
  });
  ...
  return {
    refresh() {
      ...
      try {
        ...
        for (let ancestor = source.parentElement; ancestor; ancestor = ancestor.parentElement) {
          ...
          bottomSpace += (parseFloat(css.paddingBottom) || 0) + (parseFloat(css.borderBottomWidth) || 0) + (parseFloat(css.marginBottom) || 0);
        }
        const maxY = Math.max(0, Math.round(documentTop + source.getBoundingClientRect().height + bottomSpace) - innerHeight);
    ...
    frame.style.cssText = 'position:absolute;inset:0;overflow:hidden;background:var(--bg,#0b0b0f);';
    ...
    requestAnimationFrame(() => retainedOffsets.forEach(({ x, y }, i) => displayedNodes[i]?.scrollTo({left:x,top:y,behavior:'instant'})));
    return frame;
  } };
}

export interface SwipePreview { move(distance: number): void; finish(commit: boolean): Promise<void>; release(): Promise<void>; dispose(): void }
export function previewHistory(previous: PageSnapshot, direction: 1 | -1 = 1): SwipePreview | null {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;overflow:hidden;z-index:9;pointer-events:none;background:var(--bg,#0b0b0f);';
  ...
  return {
    ...
    async finish(commit) {
      ...
      const options = { duration: matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : 160, easing:'cubic-bezier(.2,.7,.2,1)', fill:'forwards' as const };
      await Promise.all([
        front.animate([{transform:`translateX(${direction * distance}px)`,boxShadow:front.style.boxShadow},{transform:`translateX(${direction * target}px)`,boxShadow:commit?'none':front.style.boxShadow}],options).finished.catch(() => {}),
        underneath.animate([{transform:`translateX(${direction * -0.18*(width-distance)}px)`},{transform:`translateX(${direction * -0.18*(width-target)}px)`}],options).finished.catch(() => {}),
      ]);
    },
    async release() {
