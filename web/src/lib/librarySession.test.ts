...
const fakeLog = () => ({ moved: false, settings: vi.fn(), refresh: vi.fn().mockResolvedValue(true) });

it('retries an initially failed open without discarding a recovered log', async () => {
  const log = fakeLog();
  const open = vi.spyOn(LibraryLog, 'open').mockResolvedValueOnce(null).mockResolvedValue(log as unknown as LibraryLog);
...
it('refreshes remote configuration only when settings changed and serializes refreshes', async () => {
  ...
  let finish!: () => void;
  log.refresh.mockImplementationOnce(() => new Promise(resolve => {
    finish = () => { log.settings.mockReturnValue({ changed: true }); resolve(true); };
  }));
...
it('keeps a single foreground retry loop independent of the active route and cleans it up', async () => {
  ...
  const session = new LibrarySession('test');
  await session.opened;
  const moved = vi.fn(), stop = session.start(moved);
  await session.refresh();
  ...
  await vi.advanceTimersByTimeAsync(30_000);
  expect(log.refresh).toHaveBeenCalledTimes(initial + 1);
  doc.hidden = false; log.moved = true;
