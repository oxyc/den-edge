...
interface NamedLibrary { displays: Title[]; shapes: Map<string, Shape> }
type Ref = { type: MediaType; id: number };
interface NamingRun { key: string; pending: Map<string, Promise<void>> }
...
export async function nameLibraryTitles(
  session: NamedLibrary, refs: Ref[], key: string, lookup: typeof fetchDetails = fetchDetails,
): Promise<void> {
  ...
  const worker = async () => {
    for (let ref = queue.shift(); ref; ref = queue.shift()) {
      ...
      if (!work) {
        const requested = ref;
        work = Promise.resolve().then(() => lookup(requested, key)).then(found => {
        ...
        }).catch(() => {}).finally(() => current.pending.delete(id));
