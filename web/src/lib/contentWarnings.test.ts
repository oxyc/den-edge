...
it('shows only opted-in, confirmed non-spoiler warnings', () => {
  const warning = (id: number, yesSum: number, noSum: number, extra = {}) => ({
    yesSum,
    noSum,
    topic: { id, name: `Warning ${id}`, TopicCategory: { name: 'Animal Death' }, ...extra },
  });
  const body = {
    topicItemStats: [
      warning(1, 5, 1),
      warning(2, 1, 0, { isSpoiler: true }),
      warning(3, 0, 0),
      warning(4, 1, 2),
      warning(5, 3, 0, { TopicCategory: { name: 'Sex' } }),
    ],
  };
...
it('does not fetch without opt-in and never attaches warnings from an unrelated search result', async () => {
  ...
  const network = (async (input) => {
    ...
    return Response.json({
      items: [{ id: 2, name: 'Unrelated', releaseYear: '2020', imdbId: 'tt2' }],
    });
  }) as typeof fetch;
  expect(await fetchWarnings(detail, 'key', [], undefined, network)).toBeNull();
  expect(paths).toHaveLength(0);
  expect(await fetchWarnings(detail, 'key', ['Sex'], undefined, network)).toBeNull();
