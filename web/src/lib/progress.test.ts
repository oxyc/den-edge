...
describe('updateProgress', () => {
  it('puts a movie in progress, and seen past the credits', () => {
    ...
    expect([playing.status.value, playing.resume]).toEqual([
      'inProgress',
      { value: 0.4, at: at(1000), viewing: 0, seconds: 2880 },
    ]);
  ...
  it('starts a new viewing when a finished movie is played again', () => {
    ...
    expect([replay.status.value, replay.resume.value, replay.resume.viewing]).toEqual([
      'inProgress',
      0.1,
      1,
    ]);
  ...
  it('moves an episode, with the same replay rule', () => {
    const episode = updateEpisodeProgress(
      blankEpisode({ type: 'tv', id: 2 }, 1, 3),
      0.5,
      1500,
      at(1000),
    );
