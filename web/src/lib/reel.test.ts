...
const answering = (body: unknown, status = 200): typeof fetch =>
  (async (input) => {
    if (String(input) !== '/reel/cfg/meta/movie/tt0111161.json') return new Response('{}', { status: 404 });
...
const meta = {
  meta: {
    id: 'tt0111161',
    links: [
      { name: 'Trailer', category: 'Trailer', trailers: 'http://192.168.86.193:8092/play/abc123.mp4?s=tag&i=iid' },
      { name: 'Trailer', category: 'Trailer', trailers: 'http://192.168.86.193:8092/play/def456.mp4?s=tag2' },
    ],
  },
};
...
describe('trailerURL', () => {
  ...
  it('takes reel’s best trailer and points it at an address this page can reach, signature and all', async () => {
    expect(await ask(ROUTES, answering(meta))).toBe('https://pve.example:8443/reel/play/abc123.mp4?s=tag&i=iid');
  });

  it('stays on the LAN address when the page itself is plaintext', async () => {
    const onLan: Routes = { reel: [{ url: 'http://192.168.86.193:8092' }] };
    expect(await ask(onLan, answering(meta), false)).toBe('http://192.168.86.193:8092/play/abc123.mp4?s=tag&i=iid');
  });
