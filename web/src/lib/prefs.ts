...
export function readPlugins(row: SettingsRow | undefined): string[] {
  return Object.entries(row?.values ?? {})
    .filter(
      ([, stamped]) => stamped.value !== null && 'bool' in stamped.value && stamped.value.bool,
    )
    .map(([url]) => url)
    .sort();
}
...
export function isHidden(
  ...
  if (
    ...
    title.year < prefs.minReleaseYear
  ) {
    return true;
  }
...
export function readDetailPrefs(
  ...
  const region =
    country && 'string' in country && /^[a-z]{2}$/i.test(country.string)
