...
| `WEB_HOSTS` | unset | the web app's public names (comma-separated, e.g. `d.oxy.fi`, behind Cloudflare Access). A request for one gets the web app, `/health`/`/version`, `/web-config` and the routes the web app calls (`/pair/…`, `/lib/…`, `/inbox/append`) — never what only a TV does (draining its inbox, unlinking, `/sync`), `/config` or `/metrics` |
