...
| `TRUSTED_PROXIES` | unset | proxies whose report of the visitor's address counts (comma-separated IPs) — the host running `tailscale serve`, `cloudflared`. Behind one, the per-address limits read the last `X-Forwarded-For` entry, and `CF-Connecting-IP` first from an entry written `cf:<ip>` — `cloudflared`'s, since Cloudflare sets that header while `tailscale serve` passes on whatever a peer wrote in it; from anyone else those headers are ignored. Unset, every visitor through a proxy shares its limit |
